...
export default async function PublicCatalogPage({
  ...
  return (
    ...
    >
      ...
      {/* CTA */}
      <section className="relative mt-20 p-6 sm:p-10 md:p-14 bg-[var(--navy)] rounded-3xl overflow-hidden group">
        ...
        <div className="absolute top-0 right-0 w-1/2 h-full bg-gradient-to-l from-[var(--navy-light)] to-transparent skew-x-12 translate-x-32 group-hover:translate-x-10 transition-transform duration-1000 ease-out" />
        <div className="relative z-10 flex flex-col md:flex-row items-center justify-between gap-8">
          ...
          <div className="flex flex-col sm:flex-row gap-3 w-full md:w-auto shrink-0">
            <Link href="/guest/book" className="inline-flex items-center justify-center bg-[var(--gold)] text-[var(--navy)] hover:bg-[var(--gold-bright)] shadow-xl font-bold h-14 px-8 rounded-full text-lg transition-colors">
              Guest Booking <ArrowRight size={18} className="ml-2" />
            </Link>
            <Link href="/patron/register" className="inline-flex items-center justify-center bg-transparent border border-white/20 text-white hover:bg-white/10 h-14 px-8 rounded-full font-medium text-lg transition-colors">
              Create Account
            </Link>
          </div>
        </div>
...
function EmptyState({ icon, title }: { icon: React.ReactNode; title: string }) {
  return (
    <div className="py-20 px-6 text-center bg-white/40 backdrop-blur-md border border-dashed border-slate-300 rounded-3xl mt-4">
      ...
      <p className="text-slate-500 max-w-md mx-auto">Check back soon — our team is adding more options.</p>
    </div>
  );
}
